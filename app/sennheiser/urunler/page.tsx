...
export default function SennheiserMK4Page() {
  ...
  // High-fidelity image paths extracted from your preload tags
  const productImages = [
  ...
  return (
    <div className="min-h-screen bg-white text-[#1a1a1a] font-sans selection:bg-black selection:text-white">
      ...
      {/* --- HERO SECTION --- */}
      {/* The --sticky-top-height: 69px from your code is used here for top padding */}
      <main className="flex h-screen w-full overflow-hidden border-b border-gray-100">
        
        {/* LEFT PANEL: The Grey Gallery (ProductHero_product-hero-splide) */}
        <div className="w-1/2 flex flex-col h-full bg-[#f0f0f2] relative pt-[69px]">
          <div className="flex-1 flex items-center justify-center w-full relative p-12 lg:p-24"> 
            <div className="relative w-full h-full max-w-[500px] aspect-square transition-transform duration-700 ease-out">
              <Image 
            ...
            {/* NAVIGATION: Exact Sennheiser Splide Cluster */}
            <div className="absolute bottom-12 right-12 flex items-center gap-6 z-30">
              ...
              <div className="flex gap-2">
                <button 
                  ...
                  className="w-[54px] h-[54px] rounded-full bg-white flex items-center justify-center hover:bg-gray-50 transition-all shadow-sm active:scale-95 disabled:opacity-20"
                >
        ...
        {/* RIGHT PANEL: Product Detail Section */}
        <div className="w-1/2 flex flex-col h-full bg-white relative px-12 lg:px-24 pt-[69px] justify-center">
          
          <nav className="flex items-center space-x-2 text-[12px] text-gray-400 mb-10 font-medium tracking-tight">
          ...
          <div className="max-w-md">
            ...
            <p className="text-[14px] font-bold mb-12 flex items-center gap-2">
              <span className="w-2 h-2 bg-green-500 rounded-full" />
              Delivery time: 3-5 days
            </p>

            <div className="flex items-center gap-4">
              <button className="flex-1 bg-black text-white py-5 rounded-full text-[14px] font-bold hover:bg-[#333] transition-all tracking-wide uppercase active:scale-[0.98]">
                Add to cart
              </button>
      ...
      <section className="py-32 bg-[#f6f6f7] px-8 lg:px-24">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-[40px] font-bold mb-16 tracking-tight">Technical Specifications</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-20 border-t border-gray-200">
            {[
              { l: "Pick-up pattern", v: "cardioid" },
              { l: "Frequency response", v: "20 Hz to 20 kHz" },
              { l: "Sensitivity", v: "25 mV/Pa" },
              { l: "Max. SPL", v: "140 dB" },
            ].map((spec, i) => (
              <div key={i} className="flex justify-between py-6 border-b border-gray-200">
                <span className="font-bold text-[#a0a0a0] uppercase tracking-widest text-[11px] self-center">{spec.l}</span>
                <span className="text-right font-bold text-black text-[15px]">{spec.v}</span>
              </div>
            ))}
          </div>
        </div>
