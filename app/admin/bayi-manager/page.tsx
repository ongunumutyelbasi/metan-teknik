...
export default function BayiManager() {
    ...
    return (
        <div className="max-w-[1400px] mx-auto px-4 pb-10">
            ...
            <div className="flex items-center justify-between bg-white border-b border-x border-slate-200 px-4 py-2 rounded-b-sm sticky top-0 z-50 mb-6 shadow-sm">
                <div className="flex items-center gap-3">
                    <div className="p-1.5 bg-slate-100 text-slate-600 rounded-sm"><Building2 size={16} /></div>
                    <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-0.5">Bayi Yönetimi</p>
                        <p className="text-xs font-bold text-slate-900 leading-none">{fields.length} Bölge</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <button type="button" onClick={() => append({ id: `sec-${Date.now()}`, title: 'Yeni Bölge', dealers: [] })} className="h-8 px-3 text-[10px] font-bold text-slate-600 hover:bg-slate-100 rounded-sm uppercase cursor-pointer">Bölge Ekle</button>
                    <Button onClick={handleSubmit(onSubmit)} disabled={!isDirty || isSaving} className={`h-8 font-bold uppercase text-[10px] px-4 rounded-sm border-none flex items-center gap-2 ${!isDirty ? 'bg-slate-100 text-slate-400' : 'bg-metan-orange text-white cursor-pointer hover:bg-orange-600'}`}>
                        <Save size={14} /> {isSaving ? 'KAYDEDİLİYOR' : 'KAYDET'}
                    </Button>
                </div>
            </div>
...
const SortableSection = ({ id, index, register, remove, control, setValue, watch, library, onRefreshLibrary }: SortableSectionProps) => {
    ...
    return (
        <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, zIndex: isDragging ? 10 : 1 }} className={`bg-slate-50/50 border border-slate-200 p-4 rounded-sm ${isDragging ? 'opacity-30' : ''}`}>
            <div className="flex items-center gap-2 mb-4">
                <div {...attributes} {...listeners} className="cursor-grab p-1 text-slate-400 hover:text-slate-600"><GripVertical size={18} /></div>
                <div className="flex flex-col flex-1">
                    <label className="text-[8px] font-bold text-slate-400 uppercase mb-0.5">Bölge Başlığı</label>
                    <input {...register(`sections.${index}.title`)} className="text-sm font-bold text-slate-800 bg-transparent border-none outline-none focus:text-metan-orange w-full" />
                </div>
                <button type="button" onClick={() => remove(index)} className="p-1 text-slate-300 hover:text-red-600 transition-all cursor-pointer"><Trash2 size={16} /></button>
            </div>
                ...
                <SortableContext items={fields} strategy={verticalListSortingStrategy}>
                    <div className="space-y-2">
                        {fields.map((field, dIdx) => (
                            <SortableDealer key={field.id} id={field.id} index={dIdx} sectionIndex={index} register={register} remove={removeDealer} watch={watch} setValue={setValue} library={library} onRefreshLibrary={onRefreshLibrary} />
                        ))}
                        <button type="button" onClick={() => append({ id: `dlr-${Date.now()}-${Math.random()}`, name: '', logo: '', city: '', address: '', phone: '', website: '', websiteLabel: 'Satın Al', googleMapsUrl: '' })} className="w-full py-2 border border-dashed border-slate-300 text-[9px] font-bold text-slate-400 uppercase tracking-wider hover:bg-white hover:text-metan-orange rounded-sm transition-all flex items-center justify-center gap-2 mt-2 cursor-pointer">
    ...
    return (
        <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, zIndex: isDragging ? 20 : 1 }} className={`bg-white border border-slate-200 rounded-sm p-2 flex gap-3 shadow-sm ${isDragging ? 'opacity-50 ring-1 ring-metan-orange/50' : ''}`}>
            ...
            <div className="flex flex-col gap-1 shrink-0 border-r border-slate-100 pr-3 w-24 relative" ref={dropdownRef}>
                ...
                <div onClick={() => setIsOpen(!isOpen)} className="group relative w-full h-16 bg-slate-50 border border-slate-200 rounded-sm flex items-center justify-center cursor-pointer hover:border-metan-orange transition-all">
                    {logoPath ? <img src={logoPath} className="w-full h-full object-contain p-2" alt="" /> : <ImageIcon size={20} className="text-slate-200" />}
                    <div className="absolute bottom-0 right-0 p-0.5 bg-white/80 rounded-tl-sm border-t border-l border-slate-200 group-hover:bg-metan-orange group-hover:border-metan-orange transition-colors">
                        <ChevronDown size={8} className="text-slate-400 group-hover:text-white" />
                    </div>
                </div>

                {isOpen && (
                    <div className="absolute top-full left-0 mt-2 w-64 bg-white border border-slate-200 shadow-2xl z-[60] rounded-sm p-3">
                        ...
                        <div className="grid grid-cols-4 gap-3 max-h-56 overflow-y-auto p-1 scrollbar-thin">
