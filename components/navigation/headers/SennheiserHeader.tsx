  ...
  return (
    <header className='fixed top-0 left-0 w-full z-50 pt-5 px-5'>
      <div className='max-w-full mx-auto flex items-center h-[52px]'>
        ...
        {/* LOGO AREA (Desktop) */}
        <div className='hidden lg:flex items-center shrink-0 transition-all duration-500 mr-8'>
          <div className={`transition-all duration-500 overflow-hidden ${isScrolled ? 'w-0 opacity-0' : 'w-[155px] opacity-100'}`}>
            <Link href='/sennheiser' className='group'>
              <svg viewBox='0 0 155 20' className={`h-4 w-[155px] fill-current transition-colors duration-500 ${navTheme === 'light' ? 'text-white' : 'text-black'} group-hover:text-brand-blue`}>
        ...
        <nav className={`relative hidden lg:flex items-center h-full rounded-full bg-sennheiser-gray transition-all duration-700 ease-in-out px-8 ${isScrolled ? 'pl-4' : ''}`}>
          {/* Animated S-Logo Icon within the Pill when Scrolled */}
          ...
          <ul className='flex space-x-6 text-[13px] font-medium whitespace-nowrap'>
            {links.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className='text-dark-gray hover:text-brand-blue transition-colors duration-300'>
                  {link.name}
                </Link>
              </li>
            ))}
