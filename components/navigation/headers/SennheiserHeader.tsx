...
export default function SennheiserHeader() {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('sennheiser-subnav-sticky', handleSticky);
  ...
  return (
    ...
    >
      <div className='max-w-full mx-auto flex items-center h-[52px]'>
        ...
        <div className={`hidden lg:flex items-center shrink-0 transition-all duration-500 ${isScrolled ? 'w-0 opacity-0' : 'w-[155px] opacity-100 mr-5'}`}>
          <Link href='/sennheiser' className='group'>
            <svg viewBox='0 0 155 20' className={`h-4 w-[155px] fill-current transition-colors duration-500 ${navTheme === 'light' ? 'text-white' : 'text-black'} group-hover:text-brand-hover-blue`}>
        ...
        <nav className={`relative hidden lg:flex items-center h-full rounded-full bg-sennheiser-gray transition-all duration-700 ease-in-out px-4`}>
          ...
          <ul className='antialiased subpixel-antialiased flex space-x-[22px] text-[0.65rem] font-normal whitespace-nowrap'>
            {links.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className='text-dark-gray hover:text-brand-hover-blue transition-colors duration-300'>
                  {link.name}
                </Link>
              </li>
            ))}
        ...
        <div className='ml-auto flex items-center h-full'>
          ...
          <div className='lg:hidden flex items-center gap-4 z-[70]'>
            <button 
              ...
              className='w-10 h-10 flex items-center justify-center bg-sennheiser-gray rounded-full text-dark-gray'
            >
              {isMobileMenuOpen ? <X size={20} /> : <Menu size={20} />}
            </button>
            
            <Link href='/sennheiser' className='group'>
              <svg viewBox='0 0 27.82 19.14' className={`h-6 w-auto fill-current transition-colors duration-500 ${isMobileMenuOpen ? 'text-black' : (navTheme === 'light' ? 'text-white' : 'text-black')} group-hover:text-brand-hover-blue`}>
                <path d='M14.61,0H0v14.64h1.81V1.81h11.11c.57-.85,1.14-1.43,1.68-1.81h0ZM13.21,19.14h14.61V4.51h-1.81v12.8h-11.11c-.54.86-1.11,1.45-1.68,1.84h0ZM14.33,16.5c-1.61,2.62-3.11,2.64-4.95,2.64H0v-3.71h2.93c1.94,0,3.5-.98,4.33-2.38L13.52,2.64C15.1.03,16.6,0,18.44,0h9.38v3.68h-2.93c-1.94,0-3.5.98-4.33,2.41l-6.24,10.41h0Z' />
              </svg>
            </Link>
          </div>
          ...
          <nav className='flex flex-col space-y-6 mb-12'>
            {links.map((link) => (
              <Link 
                ...
                className='text-2xl antialiased subpixel-antialiased font-semibold text-black hover:text-brand-hover-blue transition-colors'
              >
                {link.name}
              </Link>
            ))}
