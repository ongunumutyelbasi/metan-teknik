...
export default function MergingHeader() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 pt-5 px-5">
      <div className="max-w-full mx-auto flex items-center relative">
        ...
        <nav className={`relative flex items-center h-[52px] rounded-full bg-[#f4f4f6] transition-all duration-700 ease-in-out ${isScrolled ? 'ml-0 pl-4 pr-8' : 'ml-[175px] pl-[20px] pr-8'}`}>
          <div className={`transition-all duration-500 flex items-center overflow-hidden ${isScrolled ? 'w-[32px] opacity-100 mr-2' : 'w-0 opacity-0'}`}>
            <Link href="/sennheiser" className="group flex items-center shrink-0">
              <svg viewBox="0 0 27.82 19.14" className="h-5 w-[27px] fill-current text-black group-hover:text-brand-blue transition-colors duration-300">
