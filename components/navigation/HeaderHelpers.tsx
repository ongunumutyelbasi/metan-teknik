...
export const BrandsDropdown = () => {
  ...
  return (
    <div className='relative group'>
      ...
      {/* Container: Adjusted to w-48, rounded-xl, and p-1.5 for a tighter look */}
      <div className='absolute right-0 top-full mt-1 w-max bg-white rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 overflow-hidden border border-gray-100 p-1.5'>
        {brands.map((brand) => (
          <Link 
            key={brand.name} 
            href={brand.href} 
...
export const SearchButton = ({ hoverClass }: { hoverClass: string }) => (
  <button className={`w-[52px] h-[52px] cursor-pointer rounded-full flex items-center justify-center bg-[#f4f4f6] text-[#545252] transition-all duration-300 ${hoverClass} hover:text-white`}>
