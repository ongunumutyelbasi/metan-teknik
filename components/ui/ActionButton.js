...
export default function ActionButton({ 
  ...
  return (
    <button 
      onClick={onClick}
      className={`antialiased subpixel-antialiased cursor-pointer bg-black text-white px-6 py-3 rounded-full text-[0.65rem] font-medium flex items-center space-x-2 hover:bg-brand-hover-blue transition-all duration-300 w-fit group ${className}`}
    >
      {text && <span>{text}</span>}
      {IconComponent && (
